...
bool ParqueEstacionamento::adicionaCliente(const string &nome) {
    if (clientes.size() == numMaximoClientes) return false; // parque cheio
    if (posicaoCliente(nome) != -1) return false; // porque ja está dentro do parque
    InfoCartao info;
    info.nome = nome; info.presente = false;
